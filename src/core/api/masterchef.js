...
import { liquidityPositionSubsetQuery, pairQuery, pairSubsetQuery } from "../queries/exchange";
import { poolHistoryQuery, poolIdsQuery, poolQuery, poolUserQuery, poolsQuery } from "../queries/masterchef";
...
export async function getPoolIds(client = getApollo()) {
  ...
  await client.cache.writeQuery({
    query: poolIdsQuery,
    data: {
      pools: pools.filter((pool) => !POOL_DENY.includes(pool.id) && pool.allocPoint !== "0"),
    },
  });
...
export async function getPools(client = getApollo()) {
  ...
  await client.cache.writeQuery({
    ...
    data: {
      pools: pools
        .filter((pool) => !POOL_DENY.includes(pool.id) && pool.allocPoint !== "0")
        .map((pool) => {
