...
import CandleStickChart from "../CandleChart/secondaryWithState";
...
const PairChart = ({ address, color, base0, base1, chartFilter, setchartFilter }) => {
  //const [chartFilter, setchartFilter] = useState(CHART_VIEW[view]);
  ...
  function valueFormatter(val) {
    if (chartFilter.chartView === CHART_VIEW.RATE0) {
      return (
        formattedNum(val) +
        `<span style="font-size: 12px; margin-left: 4px;">${formattedSymbol1}/${formattedSymbol0}<span>`
      );
    }
    if (chartFilter.chartView === CHART_VIEW.RATE1) {
      return (
        formattedNum(val) +
  ...
  return (
    <ChartWrapper>
      {chartFilter.chartView === CHART_VIEW.LIQUIDITY && (
      ...
      {chartFilter.chartView === CHART_VIEW.RATE1 &&
        (hourlyRate1 ? (
      ...
      {chartFilter.chartView === CHART_VIEW.RATE0 &&
        (hourlyRate0 ? (
          <ResponsiveContainer aspect={aspect} ref={ref}>
            <CandleStickChart
              data={hourlyRate0}
      ...
      {chartFilter.chartView === CHART_VIEW.VOLUME && (
        <ResponsiveContainer aspect={aspect}>
          <BarChart
      ...
      <OptionsRow>
        <AutoRow gap="6px" style={{ flexWrap: "nowrap" }}>
          <OptionButton
            active={chartFilter.chartView === CHART_VIEW.LIQUIDITY}
            onClick={() => {
              setTimeWindow(timeframeOptions.ALL_TIME);
              setchartFilter({ tab: chartFilter.tab, chartView: CHART_VIEW.LIQUIDITY });
            }}
          >
            Liquidity
          </OptionButton>
          <OptionButton
            active={chartFilter.chartView === CHART_VIEW.VOLUME}
            onClick={() => {
              setTimeWindow(timeframeOptions.ALL_TIME);
              setchartFilter({ tab: chartFilter.tab, chartView: CHART_VIEW.VOLUME });
            }}
          >
            Volume
          </OptionButton>
          <OptionButton
            active={chartFilter.chartView === CHART_VIEW.RATE0}
            onClick={() => {
              setTimeWindow(timeframeOptions.WEEK);
              setchartFilter({ tab: chartFilter.tab, chartView: CHART_VIEW.RATE0 });
            }}
          >
            {pairData.token0 ? formattedSymbol1 + "/" + formattedSymbol0 : "-"}
          </OptionButton>
          <OptionButton
            active={chartFilter.chartView === CHART_VIEW.RATE1}
            onClick={() => {
              setTimeWindow(timeframeOptions.WEEK);
              setchartFilter({ tab: chartFilter.tab, chartView: CHART_VIEW.RATE1 });
            }}
          >
