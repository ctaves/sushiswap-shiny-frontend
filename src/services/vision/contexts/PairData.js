...
export function usePairData(pairAddress) {
  ...
  console.log("pair_state:", pairData, 0, pairAddress);
...
/**
 * Get most recent txns for all pairs
 */
export function useAllPairTransactions() {
  const [state, { updatePairTxns }] = usePairDataContext();
  const allTxns = state?.["all"]?.txns;
  useEffect(() => {
    async function checkForTxns() {
      if (!allTxns) {
        let transactions = await getAllPairTransactions();
        updatePairTxns("all", transactions);
      }
    }
    checkForTxns();
  }, [allTxns, useAllPairTransactions]);
  return allTxns;
}

/**
 * Get most recent txns for all pairs every 3 seconds
 */
// export function useAllPairTransactionsInterval() {
//   const [state, { updatePairTxns }] = usePairDataContext();
//   const allTxns = state?.["all"]?.txns;
